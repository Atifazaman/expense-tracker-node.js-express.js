...
function createList(data) {
  ...
  if (data.typeSelect === "income") li.classList.add("incomeLi");
  else li.classList.add("expenseLi");
...
async function loadData() {
  try {
    ...
    const totalsRes = await axios.get(`${BASE_URL}/totals?${baseParams}`, {
      headers: { Authorization: `Bearer ${token}` },
    });
    ...
    document.getElementById("totalIncome").textContent = (
      totalsData.totalIncome || 0
    ).toFixed(2);
    document.getElementById("totalExpense").textContent = (
      totalsData.totalExpense || 0
    ).toFixed(2);
    document.getElementById("balance").textContent = (
      (totalsData.totalIncome || 0) - (totalsData.totalExpense || 0)
    ).toFixed(2);
...
document.body.addEventListener("click", async (e) => {
  ...
  if (e.target.id === "penBtn" || e.target.classList.contains("expenseLi")) {
...
function renderMonthlyReport(container, transactions) {
  ...
  ["Date", "Description", "Category", "Income", "Expense"].forEach(
    (text, i) => {
...
function renderYearlyReport(container, transactions) {
  ...
  sortedMonths.forEach((monthIndex, i) => {
    ...
    tdSavings.classList.add(
      monthSavings >= 0 ? "report-income" : "report-expense",
    );
